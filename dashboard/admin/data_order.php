<div class="container mt-3">
    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <?php $i = 1;
            $order = mysqli_query($kon, "SELECT * FROM tb_pelanggan WHERE status_order = 0 ORDER BY id_order DESC");
            foreach ($order as $orders) :
                $user_query =  mysqli_query($kon, "SELECT * FROM tb_user WHERE id_user = '$orders[id_user]'");
                $user = mysqli_fetch_assoc($user_query);
                $query_hartot = mysqli_query($kon, "SELECT sum(hartot_dorder) as hartot FROM tb_pesanan WHERE id_order = '$orders[id_order]'");
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <?php
            $detail_order = mysqli_query($kon, "SELECT * FROM tb_pesanan WHERE id_order = '$detRow[id_order]'");
            $query_hartot = mysqli_query($kon, "SELECT sum(hartot_dorder) as hartot FROM tb_pesanan WHERE id_order = '$detRow[id_order]'");
            $hartot = mysqli_fetch_assoc($query_hartot);
        ?>
        <table class="table table-striped">
            ...
            <tbody>
                ...
                foreach ($detail_order as $list_row) :
                    $masakan = mysqli_query($kon, "SELECT * FROM tb_menu WHERE id_masakan = '$list_row[id_masakan]' ");
